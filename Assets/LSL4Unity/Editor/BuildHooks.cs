...
{ 
    ...
    public class BuildHooks {
        ...
        {
            var buildName = System.IO.Path.GetFileNameWithoutExtension(pathToBuiltProject);

            var buildHostDirectory = pathToBuiltProject.Replace(System.IO.Path.GetFileName(pathToBuiltProject), "");

            var dataDirectoryName = buildName + "_Data";

            var pathToDataDirectory = System.IO.Path.Combine(buildHostDirectory, dataDirectoryName);

            var pluginDirectory = System.IO.Path.Combine(pathToDataDirectory, PLUGIN_DIR);
        ...
        {
            var obsoleteFile = System.IO.Path.Combine(pluginDirectory, nameOfObsoleteFile + fileEnding);

            Debug.Log("[LSL BUILD Hook] Delete obsolete file: " + obsoleteFile);

            File.Delete(obsoleteFile);

            var sourceFile = System.IO.Path.Combine(pluginDirectory, sourceName + fileEnding);

            var targetFile = System.IO.Path.Combine(pluginDirectory, LIB_LSL_NAME + fileEnding);
