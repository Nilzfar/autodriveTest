...
namespace Assets.LSL4Unity.EditorExtensions
{
    public class LSLEditorIntegration
    {
        ...
        static bool ValidateOpenLSLWindow()
        {
            ...
            lib32Available = File.Exists(System.IO.Path.Combine(root, System.IO.Path.Combine(libFolder, lib32Name + DLL_ENDING)));
            lib64Available = File.Exists(System.IO.Path.Combine(root, System.IO.Path.Combine(libFolder, lib64Name + DLL_ENDING)));

            lib32Available &= File.Exists(System.IO.Path.Combine(root, System.IO.Path.Combine(libFolder, lib32Name + SO_ENDING)));
            lib64Available &= File.Exists(System.IO.Path.Combine(root, System.IO.Path.Combine(libFolder, lib64Name + SO_ENDING)));

            lib32Available &= File.Exists(System.IO.Path.Combine(root, System.IO.Path.Combine(libFolder, lib32Name + BUNDLE_ENDING)));
            lib64Available &= File.Exists(System.IO.Path.Combine(root, System.IO.Path.Combine(libFolder, lib64Name + BUNDLE_ENDING)));

            apiAvailable = File.Exists(System.IO.Path.Combine(root, wrapperFileName));
